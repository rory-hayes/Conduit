...
Deno.serve(async (req) => {
  ...
  await supabase.from('oauth_states').insert({ workspace_id, crm: 'hubspot', state, redirect_to: redirect_to ?? '/integrations', expires_at: new Date(Date.now() + 10 * 60_000).toISOString() });
