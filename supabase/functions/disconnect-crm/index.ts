...
Deno.serve(async (req) => {
  ...
  await supabase
    ...
    .upsert({ workspace_id, crm, status: 'warning', last_checked_at: new Date().toISOString(), details_json: { disconnected: true }, updated_at: new Date().toISOString() }, { onConflict: 'workspace_id,crm' });

  await supabase.from('audit_events').insert({ workspace_id, type: 'crm_disconnected', data_json: { crm } });
